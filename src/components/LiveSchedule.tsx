    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Class cards by day — click goes straight to Arketa booking page */}
              ...
              <div key={i} className="space-y-2 min-h-[80px]">
                {dayClasses.map((cls) => {
                  ...
                  return (
                    <a
                      key={cls.id}
                      href={`https://app.arketa.co/boomerangpilates/classes/${cls.id}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="block w-full text-left p-3 rounded-sm text-sm leading-snug transition-all duration-200 cursor-pointer bg-white hover:bg-accent/10 text-charcoal border border-charcoal/15 shadow-sm hover:shadow-md hover:-translate-y-0.5"
                      title={`Book ${cls.name} — ${cls.instructor_name}`}
                    >
                      ...
                      {spotsLeft < 5 && (
                        <p className="text-xs mt-0.5 font-medium text-accent">
